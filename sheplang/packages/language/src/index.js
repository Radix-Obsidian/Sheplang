 ...
 */
export async function parseShep(source, filePath = 'input.shep') {
    ...
    if (success) {
        try {
            ...
            return { ast, appModel: null, diagnostics, success: false };
        }
    }
    return {
        ast,
        appModel: appModel, // We know it's not null if success is true
        diagnostics,
        success
    };
}
// Helper used by tests to mirror expected API that directly returns appModel
export async function parseAndMap(source, filePath = 'input.shep') {
