...
export function templateModels(app: AppModel): { path: string; content: string }[] {
  return app.datas.map((d: { name: string; fields: { name: string; type: string }[]; rules: string[] }) => {
...
export function templateActions(app: AppModel): { path: string; content: string }[] {
  return app.actions.map((a: { name: string; params: { name: string; type?: string }[]; ops: ({ kind: string; data?: string; view?: string; text?: string; fields?: Record<string, string> })[] }) => {
