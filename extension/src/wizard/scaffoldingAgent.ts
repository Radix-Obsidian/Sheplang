...
export class ScaffoldingAgent {
  ...
  public async generateProject(
    ...
    this.progressPanel.initializeSteps([
      ...
      },
      {
        ...
        description: 'Creating the main ShepLang application file'
      },
      {
        id: 'entities',
    ...
    try {
      ...
      // Step 2: Generate main application file
      this.progressPanel.startStep('main-app', ['Creating main application...', 'Setting up project structure...']);
      await this.updateProgress('Creating main application...', 25);
      await this.generateMainApplication(questionnaire, projectPath);
      this.progressPanel.completeStep('main-app', ['Main application created']);
  ...
  private async saveConfiguration(questionnaire: ProjectQuestionnaire, projectPath: string): Promise<void> {
    ...
    // Create .sheplang configuration file for project recognition
    const sheplangConfig = `# ShepLang Project Configuration
# Generated by ShepLang Wizard
...
features:
  count: ${questionnaire.features.length}
  realtime: ${questionnaire.realtime}
  api_style: ${questionnaire.apiStyle}
  deployment: ${questionnaire.deployment}
`;

    const configFilePath = path.join(projectPath, '.sheplang', 'config.sheplang');
    await vscode.workspace.fs.writeFile(
      vscode.Uri.file(configFilePath),
      Buffer.from(sheplangConfig, 'utf8')
    );

    outputChannel.info('Project configuration saved');
  }
...
`;
  ...
  /**
   * Generate the main ShepLang application file
   */
  private async generateMainApplication(questionnaire: ProjectQuestionnaire, projectPath: string): Promise<void> {
    const entities = questionnaire.entities.map(e => e.name).join(', ');
    const features = questionnaire.features.map(f => f.name).join(', ');
    
    const mainAppContent = `# ${questionnaire.projectName}
# Main ShepLang Application
# Generated by ShepLang Wizard on ${new Date().toLocaleDateString()}

app ${questionnaire.projectName} {
  # Import all entities
  ${questionnaire.entities.map(entity => `include "entities/${entity.name}.shep"`).join('\n  ')}

  # Import all flows  
  include "flows/auth/authentication.shep"
  ${questionnaire.features.map(feature => `include "flows/${this.sanitizeFolderName(feature.name)}/${this.sanitizeFolderName(feature.name)}.shep"`).join('\n  ')}

  # Import main screens
  include "screens/dashboard/main.shep"
}

# Main Application View
view MainApplication:
  header: "Welcome to ${questionnaire.projectName}"
  subtitle: "${questionnaire.description || 'A powerful application built with ShepLang'}"

  # Navigation
  navigation:
    button "Dashboard" -> show Dashboard
    button "Users" -> show UserManagement
    button "Settings" -> show Settings
    button "Reports" -> show Reports

  # Quick Actions
  section quickActions:
    title: "Quick Actions"
    button "Add New User" -> AddUser
    button "Create Report" -> CreateReport
    button "View Analytics" -> show Analytics
...
# Default Action
action showMainApplication():
  show MainApplication

# Authentication Actions
action AddUser():
  add User with name = "New User", email = "user@example.com"
  show MainApplication

action CreateReport():
  add Report with title = "New Report", createdAt = now()
  show MainApplication
...
`;
    ...
    const mainAppPath = path.join(projectPath, 'main.shep');
    const success = await this.validateAndWriteFile(mainAppPath, mainAppContent, 'application');
    
    if (success) {
      outputChannel.info('Generated main application: main.shep');
    } else {
      outputChannel.info('Generated main application with syntax errors: main.shep');
    }
  }
