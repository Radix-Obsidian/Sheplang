 ...
 * Main import command
 */
export async function importFromNextJS(): Promise<void> {
  ...
  try {
    ...
    if (!stackInfo.isNextJS) {
      vscode.window.showErrorMessage(
        'This does not appear to be a Next.js project. Please select a folder containing package.json with Next.js dependencies.'
      );
      return;
    }
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        const wizardInput = await showSemanticWizard({
        ...
        const report = generateImportReport(appModel, files);
        ...
        vscode.window.showInformationMessage(
          `✓ Successfully imported ${appModel.appName}! Review the generated .shep file and fill in TODOs.`,
          'Open Report'
        ).then(selection => {
