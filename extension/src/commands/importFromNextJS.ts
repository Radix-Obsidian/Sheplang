...
import { generateScaffold, writeScaffold } from '../generators/scaffoldGenerator';
import { showSemanticWizard } from '../wizard/semanticWizard';
import { showArchitectureWizard } from '../wizard/architectureWizard';
 ...
 * Main import command
 */
export async function importFromNextJS(context: vscode.ExtensionContext): Promise<void> {
  outputChannel.section('Next.js Import');
  ...
  try {
    ...
    outputChannel.info(`Selected project: ${projectRoot}`);

    // Step 2: Detect stack
    const stackInfo = await detectStack(projectRoot);
    if (!stackInfo.isValid) {
      vscode.window.showErrorMessage(
        'This does not appear to be a supported project. Please select a folder containing package.json with Next.js, Vite, or React dependencies.'
      );
      return;
    }
    ...
    if (stackInfo.framework === 'nextjs') {
      outputChannel.success('✓ Next.js project detected');
    } else if (stackInfo.framework === 'vite') {
      outputChannel.success('✓ Vite + React project detected');
    } else if (stackInfo.framework === 'react') {
      outputChannel.success('✓ React project detected');
    }

    if (stackInfo.hasPrisma) {
      outputChannel.success('✓ Prisma ORM detected');
    } else if (stackInfo.framework === 'nextjs') {
      outputChannel.info('⚠ No Prisma schema found - entities will be inferred from code');
    }
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        outputChannel.info(`Found: ${appModel.entities.length} entities, ${appModel.views.length} views, ${appModel.actions.length} actions`);

        // Step 4: Show wizard for refinement
        progress.report({ message: 'Launching semantic wizard...' });
        
        const wizardInput = await showSemanticWizard(context, {
        ...
        if (!wizardInput) {
          outputChannel.info('Import cancelled during wizard');
          return;
        }
        ...
        if (approach.value === 'intelligent') {
          ...
          const architecturePlan = await showArchitectureWizard(context, appModel);
          if (!architecturePlan) {
            outputChannel.info('Import cancelled during architecture planning');
            return;
          }
          ...
          outputChannel.success(`✓ Created ${project.files.length} files`);
          outputChannel.success(`Architecture: ${architecturePlan.structure}`);
          ...
          const scaffold = await generateScaffold(context, appModel, projectRoot);
          
          progress.report({ message: 'Creating organized folders...' });
          await writeScaffold(scaffold, outputFolder);
          
          outputChannel.success(`✓ Created ${scaffold.folders.length} folders`);
          for (const folder of scaffold.folders) {
            outputChannel.success(`  - ${folder.name}/ (${folder.files.length} files)`);
          }
        ...
        outputChannel.success(`✓ Created: IMPORT_REPORT.md`);
        ...
        if (generateBackend === 'Yes, generate backend') {
          ...
          if (shepthonResult) {
            ...
            outputChannel.info('⚠ Backend generation failed - you can create it manually later');
          }
        }
        ...
        vscode.window.showInformationMessage(
          `✓ Successfully imported ${appModel.appName}! ${backendFile ? 'Backend ready to test.' : 'Review .shep file and fill in TODOs.'}`,
          ...buttons
        ).then(selection => {
        ...
        outputChannel.success(`✓ Import complete!`);
      }
    );

  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    outputChannel.error('Import failed:', errorMessage);
...
async function selectProjectFolder(): Promise<string | undefined> {
  ...
  if (workspaceFolders && workspaceFolders.length > 0) {
    ...
    if (fs.existsSync(packageJsonPath)) {
      const useCurrentFolder = await vscode.window.showQuickPick(
        ['Yes', 'No, choose different folder'],
        {
...
async function detectStack(projectRoot: string): Promise<{
  ...
  // Detect framework
  let framework: 'nextjs' | 'vite' | 'react' | 'unknown' = 'unknown';
  if ('next' in allDeps) {
    framework = 'nextjs';
 ...
 */
function generateScaffoldReport(appModel: AppModel, scaffold: any): string {
  let report = '';
