 ...
 */
function parseFieldLine(line: string, enumNames: string[]): PrismaField | null {
 ...
 */
export function generateShepLangFromPrisma(schema: PrismaSchema, appName: string = 'ImportedApp'): string {
  ...
  for (const model of schema.models) {
    ...
    for (const field of model.fields) {
      ...
      if (field.isRelation) {
        ...
        if (field.onDelete) output += ` @onDelete(${field.onDelete})`;
      } else {
 ...
 */
export function importPrismaProject(projectRoot: string, appName?: string): string | null {
